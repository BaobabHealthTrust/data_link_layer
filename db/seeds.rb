...
Concept.find_by_sql("SELECT c.concept_id, n.name AS short_name, c.uuid FROM concept c LEFT OUTER JOIN concept_name n ON n.concept_id = c.concept_id WHERE name IN ('Respiratory rate','Temperature (C)','SYSTOLIC BLOOD PRESSURE','DIASTOLIC BLOOD PRESSURE','Weight (kg)','Height (cm)','Blood oxygen saturation','Pulse')").each{|c| concept = Concept.find(c.concept_id); concept.update_attributes(:uuid => lookup[c.short_name.downcase]); puts "Updated uuid for #{c.short_name} to #{concept.uuid}" }
